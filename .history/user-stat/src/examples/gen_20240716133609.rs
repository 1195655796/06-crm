...
use serde_json;
use sqlx::{postgres::PgRow, postgres::PgTypeInfo, Executor, FromRow, PgPool, Postgres, Row, Type};
...
impl<'r> Decode<'r, Postgres> for Gender {
    fn decode(value: &'r [u8]) -> Result<Self, sqlx::Error> {
        let gender_str = std::str::from_utf8(value)?;
        match gender_str {
            "Female" => Ok(Gender::Female),
            "Male" => Ok(Gender::Male),
            "Unknown" => Ok(Gender::Unknown),
