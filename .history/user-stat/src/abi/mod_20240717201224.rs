...
impl UserStatsService {
    ...
    pub async fn raw_query(&self, req: RawQueryRequest) -> ServiceResult<ResponseStream> {
        ...
        let Ok(ret) = sqlx::query_as::<_, UserStat>(&req.query)
        ...
        Ok(Response::new(Box::pin(futures::stream::iter(
            ret.into_iter().map(|user_stat| Ok(UserStat::from(user_stat))),
        ))))
    }
}
...
mod tests {
    ...
    fn query_request_to_string_should_work() {
        ...
        assert_eq!(
            sql,
            "SELECT * FROM user_stats WHERE created_at BETWEEN '2024-01-01T00:00:00+00:00' AND '2024-01-02T00:00:00+00:00'"
        );
    }
