        ...
        pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
            ...
            tokio::spawn(async move {
                while let Some(user) = user_stat_stream.next().await {
                    ...
                    if  user.last_in_app_notification.unwrap().date() < now.date() {
                        let req = SendRequest {
                            msg: Some(send_request::Msg::InApp(InAppMessage {
                                message_id : uuid::Uuid::new_v4().to_string(),
                                device_id : uuid::Uuid::new_v4().to_string(),
                                title: "Weekly Reminder".to_string(),
                                body: "Here are some interesting content.".to_string(),
                            })),
                        };
                    ...
                    if  user.last_email_notification.unwrap() < now - Duration::weeks(1) {
                        let req = SendRequest {
