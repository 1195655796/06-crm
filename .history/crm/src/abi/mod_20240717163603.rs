...
impl CrmService {
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        ...
        tokio::spawn(async move {
            while let Some(Ok(user: UserStat)) = res_user_stats.next().await {
                if user.last_visited_at < d1 {
                    let mut service = service.clone();
                    let started_but_not_finished = user
                    ...
                    if user.last_email_notification.is_none() || user.last_email_notification.unwrap() < Utc::now() - Duration::weeks(1) {
                        let req = SendRequest::new("Remind".to_string(), sender, &[user.email], &contents);
                        if let Err(e) = tx.send(req).await {
                            warn!("Failed to send message: {:?}", e);
                        }
