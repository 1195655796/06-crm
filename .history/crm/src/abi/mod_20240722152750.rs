...
impl CrmService {
    pub async fn welcome(&self, req: WelcomeRequest) -> Result<Response<WelcomeResponse>, Status> {
        ...
        pub async fn recall(&self, req: RecallRequest) -> Result<Response<RecallResponse>, Status> {
            ...
            let d1 = Utc::now() - Duration::days(req.days_inactive as i64);
            let d2 = Utc::now();
        
            // Query user stats based on gender and recent activity
            let user_stat: Vec<UserStat> = sqlx::query_as("SELECT * FROM user_stats WHERE gender = $1 AND last_visited_at BETWEEN $2 AND $3")
                .bind(req.gender.to_string()) // Assuming req.gender is an enum or string
                .bind(d1)
                .bind(d2)
                .fetch_all(&self.sqlx_pool)
                .await
            ...
            let mut scored_contents: Vec<(Content, f64)> = contents
                ...
                .map(|content| {
                    let score = w1 * content.views as f64
                        + w2 * gender_specific_views(content, &req.gender)
                        + w3 * (content.likes as f64 - content.dislikes as f64)
                        + w4 * sum_similarity(&user_stat, content)
                        + w5 * recent_views(content)
                        - decay_factor * decay_factor(content);
                    (content.clone(), score)
                })
                .collect();
        ...
        fn gender_specific_views(content: &Content, gender: &str) -> f64 {
            match gender {
                "female" => content.female_views as f64,
                "male" => content.male_views as f64,
                _ => 0.0,
            }
        }
        
        fn sum_similarity(user_stat: &[UserStat], content: &Content) -> f64 {
            user_stat.iter().fold(0.0, |acc, user| {
                if user.watched_video_ids.contains(&content.id) {
                    acc + 1.0
                } else {
                    acc
                }
        ...
        fn recent_views(content: &Content) -> f64 {
            // Placeholder for recent views scoring
            0.3
        }
        
        fn decay_factor(content: &Content) -> f64 {
            // Placeholder for decay factor calculation
            0.4
        }
