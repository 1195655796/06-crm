...
impl CrmService {
    pub async fn welcome(&self, req: WelcomeRequest) -> Result<Response<WelcomeResponse>, Status> {
        ...
        pub async fn recall(self: &'static Self, req: RecallRequest) -> Result<Response<RecallResponse>, Status> {
            let request_id = req.id.clone();
...
let mut scored_contents: Vec<(Content, f64)> = contents
    ...
    .map(|content| {
        let score = w1 * content.views as f64
            + w2 * Self::gender_specific_views(&content, gender::unknown)
        ...
        let (tx, rx) = mpsc::channel(1024);
            ...
            async move {
                while let Some(user) = user_stat_stream.next().await {
                    ...
                    let mut scored_contents: Vec<(Content, f64)> = contents.await.expect("Error getting contents").into_inner()
    .filter_map(|v| async move { v.ok() })
    .map(|content| {
        let score = w1 * content.views as f64
