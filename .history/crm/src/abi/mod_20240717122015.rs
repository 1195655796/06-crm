...
// Pass the Vec<u32> as a slice to the new_with_ids function
.materialize(MaterializeRequest::new_with_ids(&id_vec))
        let contents = self
            .metadata
            .clone()
            .materialize(MaterializeRequest::new_with_ids(id_vec))
    ...
    .collect();
                ...
                let req = SendRequest::new("Reminder".to_string(), sender, &[user.email.clone()], &contents);
                if let Err(e) = tx.send(req).await {
                    warn!("Failed to send message: {:?}", e);
                }
