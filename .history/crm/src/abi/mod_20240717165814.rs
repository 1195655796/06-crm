...
impl CrmService {
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        ...
        let mut res_user_stats: Vec<UserStat> = self.user_stats.clone().query(query).await?.into_inner();
        ...
        tokio::spawn(async move {
            while let Some(user_stat) = res_user_stats.pop() {
                if user_stat.last_visited_at < d1 {
                    let mut service = service.clone();
                    let started_but_not_finished = user_stat.started_but_not_finished.clone();
                    ...
                    if user_stat.last_email_notification.is_none() || user_stat.last_email_notification.unwrap() < Utc::now() - Duration::weeks(1) {
                        let req = SendRequest::new("Remind".to_string(), sender, &[user_stat.email.clone()], &contents);
                        if let Err(e) = tx.send(req).await {
                            warn!("Failed to send message: {:?}", e);
                        }
