...
impl CrmService {
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        ...
        let query = QueryRequest::new_with_dt("last_visited_at", None, Some(seven_days_ago));
        let mut res_user_stats = self.user_stats.clone().query(query).await?.into_inner();
        ...
        tokio::spawn(async move {
            while let Some(Ok(user)) = res_user_stats.next().await {
                ...
                let recommendations = get_recommendations(&user, &contents).await;
    ...
    async fn get_recommendations(user: &UserStat, contents: &Arc<Vec<Content>>) -> Vec<Content> {
