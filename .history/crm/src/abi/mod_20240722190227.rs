...
let mut scored_contents: Vec<(Content, f64)> = contents
    ...
    .map(|content| {
        ...
        pub fn gender_specific_views(content: &Content, gender: &str) -> f64 {
            match gender {
                "female" => 0.2,
                "male" => 0.1,
                "unknown" => 0.0,
        ...
        pub fn sum_similarity(user_stat: &[UserStat], content: &Content) -> f64 {
            0.4
        }
        
        pub fn recent_views(content: &Content) -> f64 {
            0.2
        }
        
        pub fn decay_factor(content: &Content) -> f64 {
            0.01
        }
        let (tx, rx) = mpsc::channel(1024);
            let sender = self.config.server.sender_email.clone();
