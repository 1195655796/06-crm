...
impl CrmService {
    pub async fn welcome(&self, req: WelcomeRequest) -> Result<Response<WelcomeResponse>, Status> {
        ...
        let (tx, rx) = mpsc::channel(1024);
            ...
            let mut scored_contents: Vec<(Content, f64)> = Vec::new();
while let Some(user) = user_stat_stream.next().await {
    let contents = self
        .metadata
        .clone()
    ...
    let filtered_contents: Vec<_> = contents.into_iter().filter_map(|v| v.ok()).collect();
    ...
    for content in filtered_contents {
        ...
        scored_contents.push((content, score));
    }
}
...
let recommended_contents: Vec<Content> = scored_contents.into_iter().take(9).map(|(content, _)| content).collect();
            ...
            async move {
                while let Some(user) = user_stat_stream.next().await {
                    let sender = sender.clone();
                    let tx = tx.clone();
                    let recommended_contents = recommended_contents.clone();
