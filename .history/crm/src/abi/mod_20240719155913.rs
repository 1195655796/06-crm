        ...
        pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
            ...
            tokio::spawn(async move {
                while let Some(user) = user_stat_stream.next().await {
                    ...
                    if user.last_in_app_notification.is_none() || user.last_in_app_notification.unwrap() < now.date() {
                        let req = SendRequest {
                            msg: Some(SendRequest_oneof_msg::in_app(InAppMessage {
                    ...
                    if user.last_email_notification.is_none() || user.last_email_notification.unwrap() < now - Duration::weeks(1) {
                        let req = SendRequest {
                            msg: Some(SendRequest_oneof_msg::email(EmailMessage {
                                recipient: user.email.clone(),
                                contents: contents.clone(),
                                subject: "Weekly Reminder".to_string(),
                                body: "Here are some contents you might be interested in.".to_string(),
                            })),
                        };
                        if let Err(e) = tx.send(req).await {
                    ...
                    if in_app_sent || email_sent {
                        let update_query = sqlx::query!(
                            ...
                            if in_app_sent { Some(now) } else { user.last_in_app_notification },
                            if email_sent { Some(now) } else { user.last_email_notification },
                            user.email
                        );
                        if let Err(e) = update_query.execute(&self.sqlx_pool).await {
                            warn!("Failed to update user notification timestamps: {:?}", e);
                        }
                    }
                }
            });
            ...
            // Collect the remaining items from the stream
            let remaining_reqs: Vec<_> = reqs.collect().await;
    
            // Process the remaining items
            for req in remaining_reqs {
                println!("reqs_information:{:?}", req);
            }
