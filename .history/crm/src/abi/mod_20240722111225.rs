            ...
            tokio::spawn(async move {
                while let Some(user) = user_stat_stream.next().await {
                    ...
                    if in_app_sent || email_sent {
                        ...
                        match update_query.execute(&self_arc.clone().sqlx_pool).await {
                            Ok(_) => {
                                info!("User notification timestamps updated for {}", user.email);
                            }
                            Err(e) => {
                                warn!("Failed to update user notification timestamps for {}: {:?}", user.email, e);
                            }
                        }
                    }
                }
