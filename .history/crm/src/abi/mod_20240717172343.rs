...
impl CrmService {
    pub async fn welcome(&self, req: WelcomeRequest) -> Result<Response<WelcomeResponse>, Status> {
        ...
        let res_user_stats: Vec<user_stat::examples::UserStat> = self.user_stats.clone().query(query).await?
    .into_inner()
    .map(|user_result| user_result.map(|user| user_stat::examples::UserStat::from(user)))
    .collect()
    .await;
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        let request_id = req.id;
        let last_visit_interval = req.last_visit_interval;
        let d1 = Utc::now() - Duration::days(last_visit_interval as _);
        let d2 = d1 + Duration::days(1);
        let query = QueryRequest::new_with_dt("last_visited_at", d1, d2);
        let res_user_stats: Vec<UserStat> = self.user_stats.clone().query(query).await?.into_inner().collect().await;
    
        let service = self.metadata.clone();
        let (tx, rx) = mpsc::channel(1024);
        let sender = self.config.server.sender_email.clone();
    
        tokio::spawn(async move {
            for user_stat in res_user_stats {
                if user_stat.last_visited_at < d1 {
                    let mut service = service.clone();
                    let started_but_not_finished: Vec<u32> = user_stat.started_but_not_finished.iter().map(|x| *x as u32).collect();
    
                    // Randomly select up to 9 unfinished contents
                    let contents = service
                        .materialize(MaterializeRequest::new_with_ids(&started_but_not_finished))
                        .await;
                    ...
                    let req = SendRequest::new("Remind".to_string(), sender, &[user_stat.email.clone()], &contents);
                    if let Err(e) = tx.send(req).await {
                        warn!("Failed to send message: {:?}", e);
                    }
    
                    // Send email notification once a week
                    if let Some(last_email_notification) = user_stat.last_email_notification {
                        if last_email_notification < Utc::now() - Duration::weeks(1) {
                            let req = SendRequest::new("Remind".to_string(), sender, &[user_stat.email.clone()], &contents);
                            if let Err(e) = tx.send(req).await {
                                warn!("Failed to send message: {:?}", e);
                            }
