...
impl CrmService {
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        ...
        let res_user_stats: Vec<UserStat> = self.user_stats.clone().query(query).await?.into_inner().filter_map(|result| {
            match result {
                Ok(user) => Some(UserStat::from(user)),
                Err(e) => {
                    warn!("Error fetching user stats: {:?}", e);
                    None
                }
            }
        }).collect().await;
        ...
        tokio::spawn(async move {
            for user_stat in res_user_stats {
                if let Some(last_visited_at) = user_stat.last_visited_at {
                    if last_visited_at < d1 {
