...
use jwt_simple::reexports::rand::prelude::IteratorRandom;
impl CrmService {
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        ...
        let contents = self
            .metadata
            .clone()
            .materialize(MaterializeRequest::new_with_ids(&req.id))
            .await?
            .into_inner();
    ...
    .collect();
                ...
                let req = SendRequest::new("Reminder".to_string(), sender, &[user.email], &contents);
                if let Err(e) = tx.send(req).await {
                    warn!("Failed to send message: {:?}", e);
                }
