...
impl CrmService {
    pub async fn welcome(&self, req: WelcomeRequest) -> Result<Response<WelcomeResponse>, Status> {
        ...
        //info!("emails: {:?}", emails);
        //let last_visited_at: Vec<chrono::DateTime<chrono::Utc>> = user_stat.iter().map(|stat| stat.last_visited_at).collect();
        ...
        fn gender_specific_views(content: &Content, gender: &gender) -> f64 {
            match gender {
                gender::female => 0.2,
                gender::male => 0.1,
                gender::unknown => 0.0,
        ...
        pub async fn recall(&self, req: RecallRequest) -> Result<Response<RecallResponse>, Status> {
            ...
            let d1 = Utc::now() - Duration::days(40);
            let d2 = Utc::now()- Duration::days(10);
        ...
        let (tx, rx) = mpsc::channel(1024);
            ...
            let mut user_stat_stream = futures::stream::iter(user_stat.clone());
            let cloned_sqlx_pool = self.sqlx_pool.clone();
            let mut metadata_content = self.metadata.clone();
            ...
            async move {
                while let Some(user) = user_stat_stream.next().await {
                   ...
                   ;
                    ...
                    let contents = metadata_content
                    .materialize(MaterializeRequest::new_with_ids(&req.content_ids))
                    .await.expect("Failed to materialize contents")
                    .into_inner();
                    let mut scored_contents: Vec<(Content, f64)> = contents
                    .filter_map(|v| async move { v.ok() })
            .map(|content| {
            let score = w1 * content.views as f64
                + w2 * Self::gender_specific_views(&content, &user.gender)
            ...
            (content, score)
                    ...
                    if  user.last_in_app_notification < now {
                        let req = SendRequest {
                            msg: Some(Msg::InApp(InAppMessage {
                                ...
                                title: (user.email.clone()).to_string() + " Weekly Recaller",
                                body: format!("Here are some interesting recommended contents: {:?}", recommended_contents),
                            })),
                        };
                    ...
                    if  user.last_email_notification < now {
                        let req = SendRequest {
                            msg: Some(Msg::Email(EmailMessage {
                                message_id: uuid::Uuid::new_v4().to_string(),
                                recipients: (&[user.email.clone()]).to_vec(),
                                sender: sender.clone(),
                                subject: "Weekly Recaller".to_string(),
                                body:  format!("Here are some contents you might be interested in: {:?}", recommended_contents),
