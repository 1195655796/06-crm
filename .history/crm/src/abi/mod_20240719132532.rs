...
impl CrmService {
    pub async fn welcome(&self, req: WelcomeRequest) -> Result<Response<WelcomeResponse>, Status> {
        ...
        pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
            ...
            // Fetch user stats for users who haven't visited in the last 7 days
            ...
            let mut email_reqs = Vec::new();
            let mut in_app_reqs = Vec::new();
    
            for user_stat in user_stats {
                let mut rng = ChaCha12Rng::seed_from_u64(user_stat.email.hash());
                let mut not_started: Vec<_> = user_stat.viewed_but_not_started.choose_multiple(&mut rng, 9).cloned().collect();
                if not_started.len() < 9 {
                    not_started.extend(user_stat.started_but_not_finished.choose_multiple(&mut rng, 9 - not_started.len()).cloned());
                }
    
                let contents = self
                    .metadata
                    .clone()
                    .materialize(MaterializeRequest::new_with_ids(&not_started))
                    .await?
                    .into_inner();
                ...
                let email_req = SendRequest::new("Remind".to_string(), self.config.server.sender_email.clone(), &[user_stat.email], &contents);
                email_reqs.push(email_req);
    
                let in_app_req = RemindResponse {
                    user_email: user_stat.email,
                    content_ids: not_started,
                };
                in_app_reqs.push(in_app_req);
            }
    
            self.notification.clone().send(email_reqs).await?;
            self.notification.clone().send(in_app_reqs).await?;
