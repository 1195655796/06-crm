...
impl CrmService {
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        ...
        let mut res_user_stats = user_stats.clone().query(query).await?.into_inner();
        ...
        let ids: Vec<u32> = req.id.split(',')
            .map(|id| id.trim().parse::<u32>().expect("Failed to parse ID"))
            .collect();
        ...
        // Collect the contents from the stream
        let mut contents = Vec::new();
        tokio::pin!(content_stream);
        while let Some(content) = content_stream.next().await {
            let content = content.expect("Failed to get content");
        ...
        tokio::spawn(async move {
            while let Some(Ok(user)) = res_user_stats.next().await {
                ...
                let recommendations = Self::get_recommendations(&user_stats.unwrap(), &contents).await;
