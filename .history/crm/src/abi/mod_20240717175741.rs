...
impl CrmService {
    ...
    pub async fn remind(&self, req: RemindRequest) -> Result<Response<RemindResponse>, Status> {
        ...
        let contents = self
            .metadata
            .clone()
            .materialize(MaterializeRequest::new_with_ids(&req.id))  // Adjusted to use `req.id` since `content_ids` is not available
            .await?
            .into_inner();
    
        let contents: Vec<Content> = futures::stream::iter(contents.into_iter())
            .filter_map(|v| async move { v.ok() })
            .collect()
            .await;
        let contents = Arc::new(contents);
