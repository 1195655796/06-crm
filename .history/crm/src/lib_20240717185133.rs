...
impl Crm for CrmService {
    ...
    async fn recall(
        ...
        let recall_response = RecallResponse { id };
        Ok(Response::new(recall_response))
    }
    ...
    async fn remind(
        ...
        let RemindRequest { id, last_visit_interval } = request.into_inner();

        let remind_response = RemindResponse { id };
        Ok(Response::new(remind_response))
    }
}
