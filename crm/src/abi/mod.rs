        ...
        tokio::spawn(async move {
            while let Some(user) = user_stat_stream.next().await {
                ...
                let contents = metadata_content
                    .materialize(MaterializeRequest::new_with_ids(&req.content_ids))
                    .await
                    .expect("Failed to materialize contents")
                    .into_inner();
                ...
                scored_contents.sort_by(|(_, score1), (_, score2)| {
                    score2.partial_cmp(score1).unwrap_or(Ordering::Equal)
                });

                // Select top 9 scored contents
                let recommended_contents: Vec<Content> = scored_contents
                    .into_iter()
                    .take(9)
                ...
                if user.last_in_app_notification < now - Duration::days(1) {
                    let req = SendRequest {
                        msg: Some(Msg::InApp(InAppMessage {
                            ...
                            body: format!(
                                "Here are some interesting recommended contents: {:?}",
                ...
                if user.last_email_notification < now - Duration::weeks(1) {
                    let req = SendRequest {
                        msg: Some(Msg::Email(EmailMessage {
                            ...
                            body: format!(
                                "Here are some contents you might be interested in: {:?}",
                                recommended_contents
                            ),
                        })),
                    };
